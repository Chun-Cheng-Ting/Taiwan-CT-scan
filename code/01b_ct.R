...
# ⚠ 二次跑分析時直接 read_rds 即可;要重建檔請取消下方註解區塊
#
log_progress("===== 建檔 2.7:承保檔年度家戶縣市 + 薪資四分位（F-6 + H-7 範圍 2000-2023） =====")

# H-7：範圍從 2016-2023 擴展為 2000-2023（為 H-4 extended cohort 服務）
# eligible_ids → eligible_ids_extended（涵蓋 1982-2023 出生者）
# ⚠ AMT_CUTOFF = 2000 點未依年度通膨調整；INCOME_Q 為「該年內相對排名」，
#   跨年比較需謹慎（在報告 limitations 寫清楚）
ENROL_BUILD_YEARS <- 2000:2023

# H-7 pre-flight：列出 2000-2015 各月檔存在性 + 第一個讀到的檔印 names()
log_progress("===== §2.7 pre-flight: ENROL 檔案存在性檢查 =====")
preflight_list <- list()
first_file_names_printed <- FALSE
for (y in ENROL_BUILD_YEARS) {
  for (m in 1:12) {
    file_en <- find_parquet("ENROL", y, m, suffix = "")
    preflight_list[[length(preflight_list) + 1]] <- tibble(
      year = y, month = m, exists = !is.na(file_en),
      path = if (is.na(file_en)) NA_character_ else file_en
    )
    # 第一個找到的檔印欄位名，確認 schema
    if (!first_file_names_printed && !is.na(file_en)) {
      log_progress(sprintf("第一個找到的 ENROL 檔：%d-%02d", y, m))
      log_progress(sprintf("  欄位名: %s", paste(
        names(open_dataset(file_en) |> rename_with(tolower)),
        collapse = ", "
      )))
...
preflight_df <- bind_rows(preflight_list)
log_progress(sprintf("ENROL 檔案存在數：%d / %d",
                     sum(preflight_df$exists), nrow(preflight_df)))
missing_yrs <- preflight_df |> filter(!exists)
if (nrow(missing_yrs) > 0L) {
  log_progress("⚠ 缺失月份（將跳過）：")
  print(missing_yrs |> select(year, month))
}

eligible_ids_tbl <- tibble(ID = eligible_ids_extended)

# 兩個收集 list:
#   child_ins_list:每個小孩 → 該月主投保者 id1
#   ins_amt_list  :每個主投保者 → 該月薪資與縣市
N_ENROL <- length(ENROL_BUILD_YEARS) * 12L
