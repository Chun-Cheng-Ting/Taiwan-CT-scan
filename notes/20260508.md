...
⚠ **The HR of 2.77 should not be quoted as a final estimate.** Mathews 2013 (Australia, n=10.9 M) reports IRR 1.24 for all cancers; the Korean nationwide cohort 2025 reports leukemia HR 1.07. Our HR is more than twice these published estimates from larger and longer-followed cohorts. Three design issues that plausibly inflate this estimate are detailed in §3-7 and §8.
...
#### 3-7-1. Index date asymmetry between exposed and unexposed groups *(principal concern)*

In the current pipeline (`03f §4.1`):

- Exposed children: `INDEX_DATE = FIRST_CT_DATE` (the actual date of first CT, distributed across 2016–2023)
- Unexposed children: `INDEX_DATE = STUDY_START` (fixed at 2016-01-01 for everyone)

Two consequences follow. **First**, the exposed group's at-risk window is, on average, several years shorter than the unexposed group's, because exposed children enter the at-risk period only after their first CT (plus 2-year latency). **Second**, the unexposed group is defined by "no CT during 2016–2023" — meaning a child who had their first CT in, e.g., 2020 is currently classified as **unexposed at index date 2016-01-01** if they happened to develop a malignancy before their actual CT. This admixture of "future-exposed" person-time into the unexposed comparator can dilute the unexposed risk and inflate the RR.

The proper design uses **time-varying exposure** (each child contributes unexposed person-time from study entry until their first CT, then exposed person-time from CT + latency onward) or **active comparator with matched index dates** (each unexposed child is randomly assigned an index date drawn from the empirical distribution of FIRST_CT_DATE among the exposed). Mathews 2013 used the former; Pearce 2012 implicitly avoided the issue by restricting analysis to the exposed cohort only and comparing observed-to-expected based on national rates.

**Anticipated impact of correction**: Implementing time-varying exposure would likely reduce the HR by 20–40%, based on the analogous correction in earlier cohort re-analyses.

#### 3-7-2. CT exposure rate (3.62%) is implausibly low *(principal concern)*

Our cohort yields 198,855 CT-exposed children among 5,490,109 (3.62%). For comparison, Mathews 2013 in Australia (1985–2005) found 6.2% exposure (680,211 / 10.9 M). Pediatric CT use rose sharply during the 2010s globally, so a 2016–2023 Taiwan cohort would be expected to have a *higher* exposure rate than 1985–2005 Australia, not roughly half. This raises strong concern that the CT-records pipeline is missing a substantial fraction of true exposures — most plausibly outpatient CTs that are recorded in `OPDTO` but failed to link back to patient identifiers via the 6-key `OPDTE` join.

If only a fraction of true CTs are captured, and the captured fraction is biased toward sicker patients (e.g., inpatient CTs are easier to extract than outpatient CTs because of cleaner identifier linkage), the "exposed" group becomes selectively enriched for children with comorbidities that themselves predict malignancy — a strong confounder unaccounted for by sex/age/income/urban adjustment.

**Anticipated impact of correction**: A sensitivity check that decomposes the exposed cohort by `SOURCE` (`OPDTO` vs `IPDTO`) would clarify whether selection is at play. If outpatient CTs are under-represented (e.g., < 70% of total), recovering them via revised join logic should both increase the cohort size and likely attenuate the HR.

#### 3-7-3. Two-year latency is too short for solid tumors *(principal concern)*
...
#### 3-7-4. Proportional hazards assumption violated
...
#### 3-7-5. CT type classification bug
...
#### 3-7-6. Selection bias from incomplete ENROL coverage
...
#### 3-7-7. Residual confounding by indication

Children who undergo CT are by definition sicker than those who do not. Some portion of the observed RR reflects underlying conditions that prompted both the CT and the eventual malignancy diagnosis. This confounding is partially addressed in Study 2 (sibling matching removes family-level component) and would be more rigorously addressed in Study 3 (restriction to a single indication), if Study 3 becomes feasible.
...
The same age-at-first-CT trend is observed in Study 2, with stronger magnitude:

- **Age (10,15]**: RR 3.45 (vs. 1.72 in Study 1)
- **Age (15,18]**: RR 2.43 (vs. 1.58 in Study 1)

The amplification at older ages may reflect (a) older children being a more etiologically homogenous exposure window, or (b) reverse causation that is more prominent at older ages because the CTs at age 10–18 are more frequently performed for malignancy workup than CTs at age 0–5 (which are more often performed for trauma). Distinguishing these requires the latency sensitivity analysis described in §3-7-3.
